...
class InfoAction extends CommonAction {
  ...
  public function info(){
    ...
    //热门看点
    $result_hotpoint = $article -> field('id,title') -> where('status=2') -> order('hits DESC') -> limit(8) -> select();
  ...
  public function article(){
    ...
    //读取最近更新
    $recent_updates = $infoarticle -> table('yesow_info_article as ia') -> field('ia.id,ia.title,itc.name as cname,ia.addtime') -> join('yesow_info_two_column as itc ON ia.colid = itc.id') -> where('ia.status=2') -> order('addtime DESC') -> limit(15) -> select();
    $this -> assign('recent_updates', $recent_updates);
    //读取热门点击
    $hot_clicks = $infoarticle -> table('yesow_info_article as ia') -> field('ia.id,ia.title,itc.name as cname,ia.addtime') -> join('yesow_info_two_column as itc ON ia.colid = itc.id') -> where('ia.status=2') -> order('hits DESC') -> limit(15) -> select();
	...
	$about_where .="(( ia.keyword LIKE '%{$value}%' )";
      }else{
	$about_where .=" OR ( ia.keyword LIKE '%{$value}%' )";
      }
    }
    $about_where .= ") AND (ia.id != {$id})";
