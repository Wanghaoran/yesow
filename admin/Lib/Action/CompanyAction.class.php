...
class CompanyAction extends CommonAction {
  ...
  public function editchangenotcheckcompany($id=''){
    ...
    $this -> assign('result_edit', $result_edit);
    $company = M('Company');
    $result_old_company = $company -> table('yesow_company as c') -> field('c.name,c.address,c.manproducts,c.companyphone,c.mobilephone,c.linkman,c.email,c.qqcode,c.csid, cs.name as csname, c.csaid, csa.name as csaname, c.typeid, ct.name as ctname, c.ccid, cc.name as ccname,c.website,c.keyword,c.content,c.addtime,c.updatetime') -> join('yesow_child_site as cs ON c.csid = cs.id') -> join('yesow_child_site_area as csa ON c.csaid = csa.id') -> join('yesow_company_type as ct ON c.typeid = ct.id') -> join('yesow_company_category as cc ON c.ccid = cc.id') -> where(array('c.id' => $result_edit['cid'])) -> find();
  ...
  public function rmborder(){
    ...
    $result = $rmb_order -> table('yesow_rmb_order as ro') -> field('ro.id,ro.ordernum,m.name as mname,ro.price,ro.status,ro.ischeck,p.name as pname,ro.addtime,ro.remark,ro.ispay,ro.mid') -> join('yesow_member as m ON ro.mid = m.id') -> join('yesow_payport as p ON ro.paytype = p.enname') -> where($where) -> limit($page -> firstRow . ',' . $page -> listRows) -> order('ro.addtime DESC') -> select();
    $this -> assign('result', $result);
    $this -> assign('listRows', $listRows);
    $this -> assign('currentPage', $pageNum);
    $this -> assign('count', $count);
  ...
  public function monthlymoney(){
    ...
    $pageNum = !empty($_REQUEST['pageNum']) ? $_REQUEST['pageNum'] : 1;
    $page -> firstRow = ($pageNum - 1) * $listRows;
    $result = $member_monthly -> table('yesow_member_monthly as mm') -> field('mm.id,ml.name as lname,mm.months,mm.marketprice,mm.promotionprice,mm.remark,mm.type,mm.mod') -> join('yesow_member_level as ml ON mm.lid = ml.id') -> order('ml.updatemoney ASC,mm.months ASC,mm.type ASC,mm.mod ASC') -> limit($page -> firstRow . ',' . $page -> listRows) -> where($where) -> select();
    $this -> assign('result', $result);
  ...
  public function editmonthlymoney(){
    ...
    $result = $level_monthly -> field('lid,months,marketprice,promotionprice,remark,type,mod') -> find($this -> _get('id', 'intval'));
    $this -> assign('result', $result);
    $result_level = M('MemberLevel') -> field('id,name') -> order('updatemoney ASC') -> select();
    $this -> assign('result_level', $result_level);
  ...
  public function addmemberreview(){
	...
	$this -> error($MemberReview -> getError());
      ...
      if(empty($_POST['org5_mid'])){
	$MemberReview -> mid = null;
      }
  ...
  public function deleditreviewrecord(){
    $MemberReviewRecord = M('MemberReviewRecord');
    if($MemberReviewRecord -> delete($this -> _get('id', 'intval'))){
      $this -> success(L('DATA_DELETE_SUCCESS'));
    }else{
      $this -> error(L('DATA_DELETE_ERROR'));
    }
  ...
  public function editeditreviewrecord(){
    $MemberReviewRecord = M('MemberReviewRecord');
    $_POST['addtime'] = $this -> _post('addtime', 'strtotime');
    if(!$MemberReviewRecord -> create()){
      $this -> error($MemberReviewRecord -> getError());
    }
    if($MemberReviewRecord -> save()){
      $this -> success(L('DATA_UPDATE_SUCCESS'));
    }else{
      $this -> error(L('DATA_UPDATE_ERROR'));
    }
  ...
  public function editviewreviewrecord(){
    $MemberReview = D('MemberReview');
    $MemberReviewRecord = M('MemberReviewRecord');
  ...
  public function deldealmemberreview(){
    $where_del = array();
    $where_del['id'] = array('in', $_POST['ids']);
    $MemberReview = M('MemberReview');
    if($MemberReview -> where($where_del) -> delete()){
  ...
  public function editdealmemberreview(){
    $MemberReview = D('MemberReview');
    if(!empty($_POST['org5_name'])){
      if(!$MemberReview -> create()){
	$this -> error($MemberReview -> getError());
  ...
  public function editdealmemberreviewrecord(){
    $MemberReview = D('MemberReview');
    $MemberReviewRecord = M('MemberReviewRecord');
    if(!empty($_POST['nexttime'])){
      if(!$MemberReviewRecord -> create()){
  ...
  public function addreviewformcompany(){
