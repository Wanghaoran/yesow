...
class CompanyRemindEmailModel extends Model {
  ...
  public function cutemail(){
    ...
    $next = $this -> field('id') -> where(array('sort' => array('GT', $last['sort']), 'type' => 1)) -> order('sort ASC') -> find();
    if($next){
      ...
      $frist = $this -> where('type=1') -> field('id') -> order('sort ASC') -> find();
    ...
    $next = $this -> field('id') -> where(array('sort' => array('GT', $last['sort']), 'type' => 2)) -> order('sort ASC') -> find();
    if($next){
      ...
      $frist = $this -> where('type=2') -> field('id') -> order('sort ASC') -> find();
